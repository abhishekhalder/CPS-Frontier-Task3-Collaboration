...
OUTPATH         = "./%s_freq_outfiles/marginals/" % BENCHMARK_NAME
OUTPATH_AVG     = "./%s_freq_outfiles/avg_profiles/" % BENCHMARK_NAME
###############################################################################
# REQ_TIMES        = [ 0.05*(k+1) for k in range(20)  ] # Times at which to place marginals
REQ_TIMES        = [ 1.00*(k+1) for k in range(120)  ] # Times at which to place marginals
REQ_TIMES_AVG    = [ 0.01*k     for k in range(1201) ] # Times at which to place marginals
    ...
    for membw in VALID_MEMBW:
        for freq in VALID_FREQ:
            ...
            for i in range(len(xi_vals)):
                k = 0
                outfile = (OUTPATH + "%s_%d_%d_%d_MARG%d_3dim.txt") % (BENCHMARK_NAME, ctxt[0], ctxt[1], ctxt[2], i)
            ...
            #####################################
            outfile = (OUTPATH_AVG + "%s_avgprofile_%d_%d_%d.txt") % (BENCHMARK_NAME, ctxt[0], ctxt[1], ctxt[2])
            with open(outfile, 'w') as f:
