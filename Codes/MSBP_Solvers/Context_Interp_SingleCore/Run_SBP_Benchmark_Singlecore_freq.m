...
%% Problem parameters
%==========================================================================
BENCHMARK_NAME = "mcf_s";
%==========================================================================
dfile_pfix = sprintf("../%s_freq_outfiles/marginals/%s_",BENCHMARK_NAME,BENCHMARK_NAME);
dfile_sfix = "_3dim.txt";
%==========================================================================
% VALID_CACHE = bitshift(1,1:20)-1;
% VALID_MEMBW = (1:20) * 72;
VALID_CACHE = [1 4 8 12 16 20]; % 1:20;
VALID_MEMBW = [1 4 8 12 16 20]; % 1:20;
VALID_FREQ  = 1200:100:2300;
%==========================================================================
CTXT_SET = [];

for i=2:2:20
    for j=[1200 1400 1600 1800 2100 2300]
        CTXT_SET(end+1,:) = [i,i,j];
    end
end
%==========================================================================
% marg_times    = 0:0.05:1.5;
marg_times    = 0:1.00:120.0;
% taus          = 0.0:0.01:marg_times(end)-0.01;  % Times at which to interpolate
taus          = 0.0:0.5:marg_times(end)-0.01;  % Times at which to interpolate
% taus          = 0.0:0.1:marg_times(end)-0.01;  % Times at which to interpolate
...
for k=1:numMarginal_Time
    for j=1:numMarginal_CPU
        rawD{k,j} = zeros(n, 3+3);
        for i=1:numel(CTXT_SET(:,1))
            % Get context
            % ctxt = [real(CTXT_SET(i)), imag(CTXT_SET(i))];
            ctxt = CTXT_SET(i,:);
            ctxt_s = [bitshift(1,ctxt(1))-1, ctxt(2)*72, ctxt(3)];
            
            % Read marginal file
            curr_file = sprintf("%s%d_%d_%d_MARG%d%s",dfile_pfix,ctxt_s(1),ctxt_s(2),ctxt_s(3),k-1,dfile_sfix);
        ...
        if( autoscale )
            ...
            if( max(rawD{k,j}(:,6)) ~= 0 ) c3_scale      = scaled_maxval / max(rawD{k,j}(:,6)); else c2_scale      = 1; end
        ...
        rawD{k,j}(:,6) = rawD{k,j}(:,6) * c3_scale;
        
        scaling_factors{k,j} = [instr_scale llcreq_scale llcmiss_scale c1_scale c2_scale c3_scale];
        
        locs{k,j} = [rawD{k,j}(:,1), rawD{k,j}(:,2), rawD{k,j}(:,3), rawD{k,j}(:,4), rawD{k,j}(:,5), rawD{k,j}(:,6)];
...
%         rawD{k,j}(:,3) = rawD{k,j}(:,3) / scaling_factors{k,j}(3);
        ...
        rawD{k,j}(:,6) = rawD{k,j}(:,6) / scaling_factors{k,j}(6);
    end
end
...
[x4,x5,x6]       = ndgrid(VALID_CACHE,VALID_MEMBW,VALID_FREQ);
x1 = x1(:,:)';
x2 = x2(:,:)';
x3 = x3(:,:)';
x4 = x4(:,:)';
...
pts_xi = [x1(:) x2(:) x3(:)];
pts_c  = [x4(:) x5(:)];
[x1,x2,x3,x4,x5,x6] = ndgrid(gridxi,gridxi,gridxi , VALID_CACHE,VALID_MEMBW,VALID_FREQ);
x1 = x1(:,:)';
x2 = x2(:,:)';
x3 = x3(:,:)';
x4 = x4(:,:)';
...
for k=1:numel(taus)
    ...
    for p=1:num_CPUs
        ...
        marg_c = cell(length(VALID_CACHE),length(VALID_MEMBW),length(VALID_FREQ));
        marg_c_stats = cell(length(VALID_CACHE),length(VALID_MEMBW),length(VALID_FREQ));
        for c1=VALID_CACHE
            for c2=VALID_MEMBW
                for c3=VALID_FREQ
                    marg_atc = get_xi_marg_singlecore(gridxi,gridxi,gridxi,[c1,c2,c3],pts,marg_fulldim,size(gridxi,2));
                    marg_c{c1,c2,c3} = marg_atc / numel(CTXT_SET); %marg_ctx(c1,c2);!
                    ...
                    for i=1:numel(gridxi)
                        for j=1:numel(gridxi)
                            for kk=1:numel(gridxi)
                                ...
                                marg_c_mean = marg_c_mean + marg_c{c1,c2,c3}(i,j,kk)*loc;
                            end
                        end
                    end
                    marg_c_mean = marg_c_mean / sum(marg_c{c1,c2,c3},"all");

                    % Alternative - report highest probability value
                    [~,I] = max(marg_c{c1,c2,c3},[],"all","linear");
                    [I1,I2,I3] = ind2sub(size(marg_c{c1,c2,c3}),I);
                    ...
                    marg_c_stats{c1,c2,c3} = [marg_c_mean ; marg_c_std];
                end
            end
        end
...
%==========================================================================
out_dir    = sprintf("./data_out/%s_freq_synth_profiles/", BENCHMARK_NAME);
synth_profiles = cell(numel(VALID_CACHE),numel(VALID_MEMBW),numel(VALID_FREQ));
for c1=VALID_CACHE
    for c2=VALID_MEMBW
        for c3=VALID_FREQ
            profile = zeros(numel(taus),7);
            for k=1:numel(taus)
                mean_t = marginals_stats{k,1}{c1,c2,c3}(1,:)./marginals_scales{k,1}(1:3);
                std_t  = marginals_stats{k,1}{c1,c2,c3}(2,:)./marginals_scales{k,1}(1:3);
                profile(k,:) = [taus(k), mean_t, std_t];
            end
            synth_profiles{c1,c2,c3} = profile;
            writematrix(synth_profiles{c1,c2,c3}, out_dir + BENCHMARK_NAME + "-synth_c" + num2str(bitshift(1,c1)-1) + "_" + num2str(c2*72) + "_" + num2str(c3) + ".txt");
        end
    end
end

% %% Export marginals and needful information
% save("data_out/MSBP_solution/MSBPsol_0513_iCPUs.mat","marginals", ...
