...
## Configuration ##
###############################################################################
BENCHMARK_NAME  = "streamcluster" # Choices are "dedup", "fft", "canneal" "radiosity"
###############################################################################
NUM_CACHE_PARTS = 20
NUM_MEMBW_PARTS = 20
...
INPATH_AVG      = "./%s_no_perf_duplexing_outfiles/avg_profiles/" % BENCHMARK_NAME
INPATH_SYNTH    = "./Context_Interp_SingleCore/data_out/%s_no_perf_duplexing_synth_profiles/" % BENCHMARK_NAME
OUTPATH         = "./%s_no_perf_duplexing_outfiles/synth_profile_plots/" % BENCHMARK_NAME
OUTPATH_ERRS    = "./%s_no_perf_duplexing_outfiles/error_plots/" % BENCHMARK_NAME
###############################################################################
...
for cache in VALID_CACHE:
    ...
    for membw in VALID_MEMBW:
        ...
        plt.savefig(OUTPATH + ("%s_synth-comp-c%d_%d.png" % (BENCHMARK_NAME,cache,membw)), bbox_inches="tight") # SAVE AS PNG
        fig.clear()
        plt.close(fig)
