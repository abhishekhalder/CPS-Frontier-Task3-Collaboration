#!/usr/bin/python3
#
# For all contexts [Cache,MemBW,Freq] plot all profiles superimposed on 3 plots 
...
OUTPATH         = "./%s_freq_outfiles/profile_plots/" % BENCHMARK_NAME
###############################################################################
...
for cache in VALID_CACHE:
    for membw in VALID_MEMBW:
        for freq in VALID_FREQ:
            ctxt = [cache, membw, freq]
            allfiles_data = []

            # Check if data is available for current context
            if not os.path.exists(INPATH + ("%s_%d_%d_%d_perf_1_clean.txt" % (BENCHMARK_NAME,cache,membw,freq))):
                continue
            ...
            fig.suptitle(r'Measured states $\mathbf{\xi}$ for context $\mathbf{c}=\left[%d, %d, %d\right]^{\top}$' % (ctxt[0], ctxt[1], ctxt[2]))
            ...
            for j in range(NUM_RUNS):
                for k in range(NUM_CPUS):
                    ...
                    axs[1].plot(allfiles_data[j][k][0],allfiles_data[j][k][2], linewidth=PLOTS_LINEWIDTH, color=PLOTS_LINECOLOR, alpha=PLOTS_LINEALPHA)
                    axs[2].plot(allfiles_data[j][k][0],allfiles_data[j][k][3], linewidth=PLOTS_LINEWIDTH, color=PLOTS_LINECOLOR, alpha=PLOTS_LINEALPHA)
            ...
            plt.savefig(OUTPATH + "%s_allprofiles_%d_%d_%d.png" % (BENCHMARK_NAME, ctxt[0], ctxt[1], ctxt[2]), dpi=300)
            fig.clear()
            plt.close(fig)
            ###################
