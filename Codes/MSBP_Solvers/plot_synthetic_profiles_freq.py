...
## Configuration ##
###############################################################################
BENCHMARK_NAME  = "mcf_s" # Choices are "dedup", "fft", "canneal" "radiosity"
###############################################################################
NUM_CACHE_PARTS = 20
NUM_MEMBW_PARTS = 20
# VALID_CACHE     = [ (1<<k)-1    for k in range(1,NUM_CACHE_PARTS+1) ]
VALID_CACHE     = [ (1<<k)-1    for k in [1, 4, 8, 12, 16, 20] ]
# VALID_MEMBW     = [ 72*k        for k in range(1,NUM_MEMBW_PARTS+1) ]
VALID_MEMBW     = [ 72*k        for k in [1, 4, 8, 12, 16, 20] ]
VALID_FREQ      = [ k           for k in range(1200,2301,100) ]
###############################################################################
CPU_START_INDEX = 0
NUM_RUNS        = 12 # 100                              # Valid values 1-300
NUM_CPUS        = 1    # Do not change this
INPATH          = "../../Data/freq_profiles/%s_profile/" % BENCHMARK_NAME
INPATH_AVG      = "./%s_freq_outfiles/avg_profiles/" % BENCHMARK_NAME
INPATH_SYNTH    = "./Context_Interp_SingleCore/data_out/%s_freq_synth_profiles/" % BENCHMARK_NAME
OUTPATH         = "./%s_freq_outfiles/synth_profile_plots/" % BENCHMARK_NAME
OUTPATH_ERRS    = "./%s_freq_outfiles/error_plots/" % BENCHMARK_NAME
###############################################################################
...
for cache in VALID_CACHE:
    ...
    for membw in VALID_MEMBW:
        ...
        for freq in VALID_FREQ:
            ...
            axs[0].set_xlim([0,120.0])
            axs[1].set_xlim([0,120.0])
            axs[2].set_xlim([0,120.0])
            fig.suptitle(r'$\mathbf{\xi}(t)\mid\beta$ for $\beta=(%d,%d,%d)^\top$, benchmark $\mathsf{%s}$' % (int(log(cache+1,2)),int(membw/72),int(freq),BENCHMARK_NAME), y=1)

            allfiles_data = []

            # If profiles exist for this context, plot 'em
            if os.path.exists(INPATH + ("%s_%d_%d_%d_perf_1_clean.txt" % (BENCHMARK_NAME,cache,membw,freq))):
                for j in range(NUM_RUNS):
                    i = 0
                    curr_file = INPATH + ("%s_%d_%d_%d_perf_%d_clean.txt" % (BENCHMARK_NAME,cache,membw,freq,j+1))
                    ...
                    axs[1].plot(allfiles_data[j][0][0], allfiles_data[j][0][2], linewidth=PLOTS_LINEWIDTH, color=PLOTS_LINECOLOR, alpha=PLOTS_LINEALPHA)
                    axs[2].plot(allfiles_data[j][0][0], allfiles_data[j][0][3], linewidth=PLOTS_LINEWIDTH, color=PLOTS_LINECOLOR, alpha=PLOTS_LINEALPHA)

                # Load emperical mean profile
                avg_profile_file = INPATH_AVG + ("%s_avgprofile_%d_%d_%d.txt" % (BENCHMARK_NAME,cache,membw,freq))
                curr_avg_profile = [[],[],[],[]]
            ...
            synth_profile_file = INPATH_SYNTH + ("%s-synth_c%d_%d_%d.txt" % (BENCHMARK_NAME,cache,membw,freq))
            curr_synth_profile = [[],[],[],[],[],[],[]]
            ...
            # plt.savefig(OUTPATH + ("%s_synth-comp-c%d_%d.pdf" % (BENCHMARK_NAME,cache,membw)), bbox_inches="tight") # SAVE AS PDF
            plt.savefig(OUTPATH + ("%s_synth-comp-c%d_%d_%d.png" % (BENCHMARK_NAME,cache,membw,freq)), bbox_inches="tight") # SAVE AS PNG
            fig.clear()
            plt.close(fig)
