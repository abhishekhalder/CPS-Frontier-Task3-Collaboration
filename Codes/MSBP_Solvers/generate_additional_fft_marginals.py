#!/usr/bin/python3
#
# Generate marginals from additional data fot FFT with resource alloc. [2 2]
#
...
BENCHMARK_NAME = "fft"
###############################################################################
NUM_CACHE_PARTS = 1 # 20
NUM_MEMBW_PARTS = 1 # 20
VALID_CACHE     = [ (1<<k)-1    for k in range(2,3) ]
VALID_MEMBW     = [ 72*k        for k in range(2,3) ]
###############################################################################
NUM_RUNS        = 500
NUM_CPUS        = 1                        # Do not change this
INPATH          = "../../Data/%s_profile/" % BENCHMARK_NAME
OUTPATH         = "./%s_outfiles/marginals_additional/" % BENCHMARK_NAME
# OUTPATH_AVG     = "./%s_outfiles/avg_profiles/" % BENCHMARK_NAME
###############################################################################
REQ_TIMES        = [ 0.05*(k+1) for k in range(20)  ] # Times at which to place marginals
REQ_TIMES_AVG    = [ 0.01*k     for k in range(101) ] # Times at which to place marginals
    ...
    for membw in VALID_MEMBW:
        ...
        for j in range(NUM_RUNS):
            ...
            for line in open(curr_file, 'r'):
                ...
                xi = [ float(last_line[ccpu][2]), float(last_line[ccpu][3])+float(last_line[ccpu][4]), float(last_line[ccpu][5]) ]
