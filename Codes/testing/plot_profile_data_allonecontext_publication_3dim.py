#
# Given a specific context [Cache,MemBW,PathNo] plot all 500 profiles 
# superimposed on 3 plots (1 for each state variable), in the format 
# requested by Dr. Halder for publication, combining LLC loads and stores
# into one variable, LLC requests.
#
...
fig.suptitle(r'Measured states $\mathbf{\xi}$ for context $\mathbf{c}=\left[%d, %d, y_{\rm{des}}^{%d}(x)\right]^{\top}$' % (15, 15, (PATHNO+1)))


for j in range(NUM_RUNS):
    axs[0].plot(allfiles_data[j][0],allfiles_data[j][1], linewidth=PLOTS_LINEWIDTH, color=PLOTS_LINECOLOR, alpha=PLOTS_LINEALPHA)
    axs[1].plot(allfiles_data[j][0],list( map(add, allfiles_data[j][2], allfiles_data[j][3]) ), linewidth=PLOTS_LINEWIDTH, color=PLOTS_LINECOLOR, alpha=PLOTS_LINEALPHA)
    axs[2].plot(allfiles_data[j][0],allfiles_data[j][4], linewidth=PLOTS_LINEWIDTH, color=PLOTS_LINECOLOR, alpha=PLOTS_LINEALPHA)
