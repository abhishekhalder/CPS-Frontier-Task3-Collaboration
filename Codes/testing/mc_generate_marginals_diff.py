#!/usr/bin/python3
#
# Given a directory and a list of timestamps at which we desire to place marginals,
...
# of \xi to 3. Note that we assume that all files in the given directory correspond to one HW context.
#
...
OUTPATH          = "./halder_outfiles_0409/"
OUTPATH_INTRP    = "./halder_outfiles_0409/"
CPU_STRT_IND     = 0
###################
# REQ_TIMES   = [0.0, 0.5, 2.0, 7.0, 11.0, 13.0, 15.0]                # A list of times at which we would like to place marginals (02/14)
# REQ_TIMES   = [0.0, 1.0, 2.0, 2.5, 4.0, 11.5, 12.6]                # A list of times at which we would like to place marginals (03/07)
# INTRP_TIMES = [0.8, 2.2, 8.0, 11.0, 12.0]                          # A list of times at which we would like to interpolate marginals (03/07)

REQ_TIMES   = [0.0, 0.5, 1.5, 2.5, 5.0, 9.5, 10.5]                # A list of times at which we would like to place marginals (03/29; CURR)
INTRP_TIMES = [0.8, 2.2, 7.0, 9.0, 10.0]                          # A list of times at which we would like to interpolate marginals (03/29; CURR)
...
xii_vals = [[[] for j in range(NUM_CPUS)] for i in range(len(tNi_vals))]
...
for j in range(NUM_RUNS):
    ...
    for line in open(curr_file, 'r'):
        ...
        ccpu = int(splt[1][3:]) - CPU_STRT_IND
        ...
        # Check if last line matches any of the t_Ni
        for k in range(len(tNi_vals)):
            if( (float(last_line[ccpu][0]) < tNi_vals[k]) and (tNi_vals[k] <= float(splt[0])) ):
                xii_vals[k][ccpu].append(xi)
                print("Found %f ~= %f (t_%d(i))" % (float(last_line[ccpu][0]), tNi_vals[k], k))
                break
...
for i in range(len(xi_vals)):
    for k in range(NUM_CPUS):
        # outfile = OUTPATH + "synthetic_1__MARG"+str(i)+"."+str(k+1)+"_3dim.txt"
        outfile = OUTPATH + "canneal_1__MARG"+str(i)+"."+str(k+1)+"_3dim.txt"
...
print("-----")
###############################
for i in range(len(xii_vals)):
    for k in range(NUM_CPUS):
        # outfile = OUTPATH + "synthetic_1__MARGINT"+str(i)+"."+str(k+1)+"_3dim.txt"
        outfile = OUTPATH + "canneal_1__MARGINT"+str(i)+"."+str(k+1)+"_3dim.txt"

        print("Writing file " + outfile)

        with open(outfile, 'w') as f:
            for j in range(len(xii_vals[i][k])):
                f.write("%f %f %f\n" % (xii_vals[i][k][j][0], xii_vals[i][k][j][1], xii_vals[i][k][j][2]))
###############################
