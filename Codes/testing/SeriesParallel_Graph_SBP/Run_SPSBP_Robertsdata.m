...
% marg_times    = [0.0, 1.0, 2.0, 2.5, 4.0, 11.5, 12.6];     % Times at which marginals are placed
% taus          = [0.8, 2.2, 8.0, 11.0, 12.0];               % Times at which to interpolate
...
for k=1:numMarginal_Time
    if k==1 || k==numMarginal_Time
        u{k,1} = u_saved{k,2}(1:n);
        err{k,1} = { []; [] };
    else
        for j=1:numMarginal_CPU
            u{k,j} = u_saved{k,j+1}(1:n);
            err{k,j} = { []; [] };
        end
    end
end
...
while iter_idx <= maxIter
    ...
    % tic;
    Proj = Proj1_scattered_fixed(t, j, numMarginal_Time, numMarginal_CPU, K, u);
...
for k=1:numMarginal_Time-1
    for j=1:numMarginal_CPU
        if k==1
            M{k,j} = Proj2_scattered_fixed([1,1], [2,j], numMarginal_Time, numMarginal_CPU, K, u);
        elseif k==numMarginal_Time-1
            M{k,j} = Proj2_scattered_fixed([k,j], [k+1,1], numMarginal_Time, numMarginal_CPU, K, u);
        else
            M{k,j} = Proj2_scattered_fixed([k,j], [k+1,j], numMarginal_Time, numMarginal_CPU, K, u);
        end
...
for k=1:numMarginal_Time
    ...
    if k>1 && k<numMarginal_Time
        for l=1:numMarginal_CPU
            ...
            writecell(err{k,l}, out_dir + "f1_err" + num2str(f1_ctr) + ".txt");
            f1_ctr = f1_ctr + 1;
        end
    else
        l = 1;
