...
%% Problem parameters
% =========================================================================
% dfile_pfix = "../halder_outfiles_0926/kbm_sim_32767_1080_0_t";
dfile_pfix = "../halder_outfiles_0927/kbm_sim_32767_1080_0_t";
...
addD = cell(3,1);
for k=1:num_interp
    % addD{k} = readmatrix("../halder_outfiles_0922_intermarginals/kbm_sim_32767_1080_0_t" + num2str(4*(l-1)+k) + "_3dim.txt");
...
for j=1:3
    ...
    plot3(si*ones(numel(X1),1), X1, marg, "k");
    hold on;
    % s_int = 0.002;
    % marg = spline(X1, marg, min(X1):s_int:max(X1));
    % plot3(si*ones(numel(min(X1):s_int:max(X1)),1), min(X1):s_int:max(X1), marg, "b");
    
    % Plot the measured intermediate distributions
    for k=1:num_interp
        [X1, marg] = getMarginal1D(addD{k}(:,j), nBins);
        % marg = marg/sum(marg);
        % plot3((si+(ei-si)*(k/(num_interp+1)))*ones(numel(X1),1), X1, marg, "r");
        marg = smoothdata(marg);
        plot3((si+(ei-si)*(k/(num_interp+1)))*ones(numel(X1),1), X1, marg, "r");
    end
    ...
    for k=1:num_interp
        ...
        % [X1, marg] = getMarginal1D(locs{k}(:,j), nBins*1);
        % marg = marg/sum(marg);
        % plot3((si+(ei-si)*(k/(num_interp+1)))*ones(numel(X1),1), X1, marg, "b");
        % marg = smoothdata(marg);
        plot3((si+(ei-si)*(k/(num_interp+1)))*ones(numel(X1),1), X1, marg, "b");
    end
    ...
    % marg = marg/sum(marg);
    marg = smoothdata(marg);
    plot3(ei*ones(numel(X1),1), X1, marg, "k");
    
    % Plot intermediate marginals (DEBUG)
%     [X1, marg] = getMarginal1D(rawD{si+1}(:,j), nBins);
%     % marg = marg/sum(marg);
%     marg = smoothdata(marg);
%     plot3((si+1)*ones(numel(X1),1), X1, marg, "g");
%     [X1, marg] = getMarginal1D(rawD{si+2}(:,j), nBins);
%     % marg = marg/sum(marg);
%     marg = smoothdata(marg);
%     plot3((si+2)*ones(numel(X1),1), X1, marg, "g");
