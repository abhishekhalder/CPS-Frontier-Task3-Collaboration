...
y = x;
[X,Y] = ndgrid(x,y); XX = [X(:) Y(:)]; nSample = size(XX,1); n = nSample;
%% Problem parameters
% ========================
...
while iter_idx <= maxIter
    ...
    logP_b = zeros(n,1); logP_e = zeros(n,1);
    if( t == 1 )
        % log(Ku_T)
        for i=1:n
            logP_e(i) = logsumexp( -C(i,:)/epsilon+u{numMarginal}' );
        end
        
        % K*diag(u_k)*...
        for k=numMarginal-1:-1:2
            temp = logP_e;
            for i=1:n
                temp(i) = logsumexp( -C(i,:)/epsilon+u{k}'+logP_e' );
            end
            logP_e = temp;
        end
    elseif( t == numMarginal ) 
        % log(K^\top u_1)
        for i=1:n
            logP_b(i) = logsumexp( -C(:,i)'/epsilon+u{1}' );
        end
        ...
        for k=2:numMarginal-1
            temp = logP_b;
            for i=1:n
                temp(i) = logsumexp( -C(:,i)'/epsilon+u{k}'+logP_b' );
            end
            logP_b = temp;
        end
    else
    ...
    u{t} = log(mu_snapshots{t}) - logP_b - logP_e;

    err{t}{1}(end+1) = iter_idx;
    err{t}{2}(end+1) = HilbertProjectiveMetric_logscale(u{t},u_old);
